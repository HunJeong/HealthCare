package com.example.jeonghun.heathcare.Bluetooth;
...
public class BluetoothSerialClient {
    ...
    public static BluetoothSerialClient getInstance() {
        if(sThis == null) {
            sThis = new BluetoothSerialClient(); //자기 자신을 생성
        }
        ...
        return sThis; //자기 자신을 리턴
    }

    private BluetoothSerialClient() {
        mBluetoothAdapter = BluetoothAdapter.getDefaultAdapter(); //블루투스 어답터를 정의
        mReadExecutor = Executors.newSingleThreadExecutor(); //executor 1개 생성
    ...
    public boolean scanDevices(Context context, OnScanListener OnScanListener) {
        if(!mBluetoothAdapter.isEnabled()) return false; //블루투스가 연결상태가 아니면
        if(mBluetoothAdapter.isDiscovering()) { //블루투스 발견
            mBluetoothAdapter.cancelDiscovery(); //탐색종료
            try {
                context.unregisterReceiver(mDiscoveryReceiver); //이전에 사용한 브로드캐스트 리시버를 제거
    ...
    private void connectClient() {
        try {
            mBluetoothSocket = mConnectedDevice.createRfcommSocketToServiceRecord(mUUID); //socket을 불러옴
        } catch (IOException e) {
            close();
            e.printStackTrace();
        ...
        mWriteExecutor.execute(new Runnable() { //write executer 실행
            @Override
            public void run() {
                try {
                    mBluetoothAdapter.cancelDiscovery(); //탐색 종료
